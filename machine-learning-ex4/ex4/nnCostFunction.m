...
for i = 1: m
  ...
  % Feedforward
  a1 = [1 X(i,:)]';  % a1: 401x1,  Theta1: 25x401, convert to vector

  z2 = Theta1 * a1; % z2: 25x1
  a2 = sigmoid(z2);
  a2 = [1; a2];  % a2: 26x1

  z3 = Theta2 * a2; % z3: 10x1 Theta2: 10x26
  a3 = sigmoid(z3); % h: 10x1
  ...
  s = s + (-_y' * log(a3)  - (1 - _y)' * log(1-a3));
end

% change bias feature to 0
...
J =  (1 / m) * s + (lambda / (2 * m)) * (sum(sum(Theta1.^2)) + sum(sum(Theta2.^2)));
% -------------------------------------------------------------
